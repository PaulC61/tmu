...
def main(args):    
    concept_0 = np.empty(args.number_of_features, dtype=np.uint32) 
    concept_0[:args.number_of_features//2+args.overlap//2] = 1
    concept_0[args.number_of_features//2+args.overlap//2:] = 0
    print(concept_0)
    
    concept_1 = np.empty(args.number_of_features, dtype=np.uint32) 
    concept_1[:args.number_of_features//2-args.overlap//2] = 0
    concept_1[args.number_of_features//2-args.overlap//2:] = 1
    print(concept_1)

    concept_2 = np.empty(args.number_of_features, dtype=np.uint32) 
    concept_2 = np.maximum(concept_0 - np.minimum(concept_0, concept_1), concept_1 - np.minimum(concept_0, concept_1))
    print(concept_2)

    concepts = np.empty((args.number_of_features, args.number_of_features), dtype=np.uint32)
    for i in range(args.number_of_features):
        concepts[i,:] = 1
        concepts[i,i] = 0 

    common = (1 - concept_2).nonzero()[0]
    class_0 = np.intersect1d(concept_0.nonzero()[0], concept_2.nonzero()[0])
    class_1 = np.intersect1d(concept_1.nonzero()[0], concept_2.nonzero()[0])
    ...
    for j in range(args.number_of_clauses // 2):
        ...
        l = []
        for k in range(args.number_of_features):
...
def default_args(**kwargs):
    ...
    parser.add_argument("--number-of-examples", default=20000, type=int)
